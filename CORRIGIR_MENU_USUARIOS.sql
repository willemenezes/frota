-- ============================================
-- Script para corrigir o menu "Usuários" que sumiu
...
SELECT 
  ...
  ur.role as meu_role,
  CASE 
    WHEN ur.role = 'administrador' THEN '✅ Você é ADMINISTRADOR'
    WHEN ur.role = 'gestor' THEN '⚠️ Você é GESTOR (não administrador)'
    WHEN ur.role = 'motorista' THEN '⚠️ Você é MOTORISTA (não administrador)'
    ELSE '❌ Você NÃO TEM ROLE'
  END as status
...
-- 4. Ou para tornar um usuário específico como administrador (substitua o email):
-- 
...
-- );
-- 
-- Se o usuário não tiver role ainda:
-- 
-- INSERT INTO public.user_roles (user_id, role)
